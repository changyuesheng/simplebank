...
const getAccountFoeUpdate = `-- name: GetAccountFoeUpdate :one
SELECT id, owner, balance, currency, created_at FROM accounts
WHERE id = $1 LIMIT 1
FOR NO KEY UPDATE
`

func (q *Queries) GetAccountFoeUpdate(ctx context.Context, id int64) (Account, error) {
	row := q.queryRow(ctx, q.getAccountFoeUpdateStmt, getAccountFoeUpdate, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
